 ...
 * デモモード時の固定応答集
 *
 * Gemini API の無料枠制限（429 RESOURCE_EXHAUSTED）を回避するため、
 * 商談デモ中は実API呼び出しを行わず、このファイルから事前応答を返す。
 *
 * 切り替え: .env の DEMO_MODE
 *   true  → 事前応答（このファイル）
 *   false → Gemini API 実呼び出し
 */

// ===== 固定ID（seedData.ts の DEMO_FIXED_IDS と一致させること） =====
// サーバ側からも import するためここで直接定数化（Firebaseに依存しないため）
const DEMO_IDS = {
...
// ===== 照会回答書 (generate-conference-reply) =====
...
export const DEMO_GENERIC =
  'デモモード中はAI応答が事前定義されたサンプルに切り替わっています。実APIに切り替えるには .env の DEMO_MODE を false にしてください。';
